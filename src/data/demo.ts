...
export const demoTrip: TripData = {
  ...
  travelers,
  dayPlans: [{ id: 'demo-day-today', date: today, city: 'Madrid', title: '抵达马德里', subtitle: '先放行李，再慢慢认识这座城', timeZone: 'Europe/Madrid', travelerIds: travelers.map((t) => t.id), eventIds: ['demo-event-arrive', 'demo-event-museum', 'demo-event-hotel'], taskIds: ['demo-task-ticket'], sourceIds: ['demo-source'], guidance: { status: 'suggested', route: '机场到市区后按同一区域顺序步行，减少折返。', rest: '抵达后先放行李并留出完整午餐时间。', lighter: '下雨、疲劳或晚出门时只保留入住，博物馆作为可选项。', optionalEventIds: ['demo-event-museum'], sourceIds: ['demo-source'] } }],
  events: [
