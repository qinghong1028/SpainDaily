...
- 19 unit tests for IANA local-date calculation, date clamping/ranges, schema validation, schema-version rejection/migration entry point, encrypted IndexedDB vault persistence, state preservation across content updates, traveler-isolated completion state, local-state/package encryption round-trips, missing-attachment refusal, and wrong-password rejection.
...
- Private-package validation with 70 attachments (56 original media plus 14 full-page PDF renders): encrypt, decrypt, payload hash, every attachment hash, schema/reference integrity, import into the encrypted vault, per-person schedule, original ticket opening, original-page opening, reload password gate, cold-start traveler gate, explicit lock, and offline password unlock.
